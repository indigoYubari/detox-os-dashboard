...
| 0010_kim_cards.sql | Ny tabell `kim_cards` + view `v_kim_cards` (Indigos Kim-kort). RLS: `authenticated` SELECT; maskinprinsipal (`detox_role=service`) ser kun `active`. Skriver: `scripts/sync-kim-cards.py` (service_role). | IKKE KJØRT — venter Adrians godkjenning (skjemaendring) |
