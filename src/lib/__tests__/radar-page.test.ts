// Render-test av /radar (funn-utforskeren i den nye flaten) med datalaget mocket
...
async function render(searchParams?: Record<string, string>) {
  const mod = await import("../../app/(ny)/radar/page")
...
describe("/radar", () => {
  ...
  it("filtrene er lenker, og valgt agent gir bare den seksjonen", async () => {
    ...
    expect(html).toContain('href="/radar?agent=indigo"')
    expect(html).toContain('href="/radar?periode=30d"')
