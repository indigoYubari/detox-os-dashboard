...
import { agentValg, funnHref, funnLede, nyesteForst, periodeValg, typeValg } from "../funn/funn"
...
describe("funnene: lenker og filtre", () => {
  it("bare avvik fra default havner i lenka, paa /funn", () => {
    expect(funnHref(DEFAULT_FILTERS, {})).toBe("/funn")
    expect(funnHref(DEFAULT_FILTERS, { periode: "30d" })).toBe("/funn?periode=30d")
    expect(funnHref({ agent: "indigo", periode: "30d", kind: "watch" }, { kind: null })).toBe(
      "/funn?agent=indigo&periode=30d",
    )
  })
